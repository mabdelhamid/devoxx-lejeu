...
const onQuizzReceived = (state: IQuizzState, action: QuizzReceivedAction) => {
  return action.quizz
    ? { ...state, quizz: action.quizz, isQuizzSubmitted: false }
    : state;
};

const onSelectedAnswer = (
  state: IQuizzState,
  action: QuizzAnswerSelectedAction
) => ({ ...state, selectedAnswer: action.answerId });

const onQuizzSubmitted = (
  state: IQuizzState,
  action: QuizzSubmittedAction
) => ({
  ...state,
  isQuizzSubmitted: true,
  isCorrectAnswer: action.result.isCorrectAnswer,
  bonusWined: action.result.bonusWined
});
...
const onHeroLoggedIn = (state: IQuizzState, action: any) => <IQuizzState>{};

const onHeroLoggedOut = (state: IQuizzState, action: any) => <IQuizzState>{};
